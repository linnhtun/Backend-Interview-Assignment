...
internal class UserConfig : WebSecurityConfigurerAdapter() {
    ...
    override fun configure(http: HttpSecurity) {
        http
            ...
            .httpBasic()
            .and()
            .csrf().ignoringRequestMatchers(RequestMatcher { request: HttpServletRequest -> "/introspect" == request.getRequestURI() })
    }
 ...
 */
@FrameworkEndpoint
internal class IntrospectEndpoint(tokenStore: TokenStore) {
    var tokenStore: TokenStore? = null

    @PostMapping("/introspect")
    @ResponseBody
    fun introspect(@RequestParam("token") token: String?): Map<String, Any?> {
        val accessToken: OAuth2AccessToken? = tokenStore?.readAccessToken(token)
        val attributes: MutableMap<String, Any?> = HashMap()
        if (accessToken == null || accessToken.isExpired()) {
            attributes["active"] = false
            return attributes
        }
        val authentication: OAuth2Authentication? = tokenStore?.readAuthentication(token)
        attributes["active"] = true
        attributes["exp"] = accessToken.getExpiration().getTime()
        attributes["scope"] = accessToken.getScope().stream().collect(Collectors.joining(" "))
        attributes["sub"] = authentication?.getName()
        return attributes
    }
